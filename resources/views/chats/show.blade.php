...
{{-- Both must run BEFORE chats.partials.thread's own <script> tags below
     (which the browser executes as soon as it reaches them, in document
     order): chat-thread-controller.js sets window.__chatThreadSignal that
     chats-show.js registers its polling/Ably teardown against, and the Ably
     SDK itself must already be defined before chats-show.js's own
     `typeof Ably === 'undefined'` check runs. Placed first for exactly
     that reason; see chat-thread-controller.js's header comment. --}}
